  ...
  const clusterInfo = await fetchClusterInfo(params.id);
    return (
      <div className="w-screen h-screen flex flex-col sm:flex-row flex-grow overflow-hidden bg-neutral-800 border border-gray-800 text-zinc-400 font-Nunito">
        <div className="sm:w-1/3 md:1/5 w-full flex-shrink flex-grow-0 bg-gray-800 items-center">
            <div className="h-full sticky top-0 pt-4 pl-4 w-full flex justify-between flex-col border border-gray-800">
                <div>
        ...
        <div className="w-full h-full flex-grow m-1 p-4 overflow-auto bg-gray-900 justify-start items-center border border-1 border-zinc-700">
          <h1 className="text-zinc-100 text-[21px] font-light font-Nunito leading-loose">Snapshot Policy</h1>
          <div>TBD</div>
        </div>
      </div>
  );
